...
const NotesPage = () => {
    ...
    return (
        <div className="container mt-5">
            <Title level={2}>Notes for {username}</Title>
            <Form onFinish={handleCreateNote} className="border p-4 rounded">
                <Form.Item
                    label="Title"
            ...
            <List
                className="mt-4"
                header={<div>My Notes</div>}
                bordered
                dataSource={notes}
                renderItem={item => (
                    <List.Item
                        actions={[
                            <Button onClick={() => handleUpdateNote(item._id, item.title, item.content)}>Update</Button>,
